...
use app\wxopen\server\ProjectWxTool;
//是否本地缓存微信开放平台账号数据
...
//        echo 'redisKey => '.$redisKey.'<br/>';
        if (isset($redisData['unique_key']) && ($redisData['unique_key'] == $redisKey)) {
            return $redisData['access_token'];
        }
        throw new WxOpenException('wxu获取平台access token失败', ErrorCode::WXOPEN_PARAM_ERROR);
    }

    private static function getAuthorizerAccessTokenOld(string $appId)
    {
        ...
        \Log\Log::log('getAuthorizerAccessTokenOld');

        $redisKey = Project::REDIS_PREFIX_WX_COMPONENT_AUTHORIZER . $appId;
        $redisData = CacheSimpleFactory::getRedisInstance()->hGetAll($redisKey);

        if (!isset($redisData['at_key'])) {
            $authorizerInfo = ProjectWxTool::getWxOpenAuthorizerInfo($appId);
            ...
            \Log\Log::log('$authorizerInfo');
            \Log\Log::log(json_encode($authorizerInfo));
            CacheSimpleFactory::getRedisInstance()->hMSet($redisKey, $authorizerInfo);
        ...
        CacheSimpleFactory::getRedisInstance()->expire($redisKey, 6500);
    ...
    private static function getAuthorizerAccessTokenNew(string $appId)
    {
        $cacheInfo = ProjectTool::getWxOpenAuthorizerCache($appId);
    ...
    {
        if (method_exists('\Tool\ProjectTool', 'getWxOpenAuthorizerCache')) {
            return self::getAuthorizerAccessTokenNew($appId);
        }

        return self::getAuthorizerAccessTokenOld($appId);
    }
